...
- **The shared-worker gap got SHARPER, not softer** (found in the handover.md session). handover.md's references/-carve-out is now retired outright: the embedded-work check has no exemption, and a stamped file in `references/` is a signal mismatch rather than an exempt handover. That closes the loophole deliberately — but it also removes the only shelter the shared-worker pattern had (reused extracted work living in `references/`, which is neither plain reference nor handover). Under the new rules such a file is unambiguously a defect with no legal home. Decide here, explicitly: either the pattern is genuinely illegal and its instances migrate to handovers or child skills, or it is legitimate and the framework owes it a home. Do NOT leave it as a standing false positive the saddler emits and readers learn to ignore.
- Terms form re-key: [plan-terms-notation.md](plan-terms-notation.md) deprecates the `:` prefix — spec-check:51's **Terms form** check becomes "bolded list entries, Title Case names".
- **The grants-transfer claim is UNVERIFIED and must be settled here, across three sites at once.** The framework asserts in FOUR places that a skill's `allowed-tools` transfer to whatever invokes it: `docs/drafthorse/framework/scaffold.md:19` (Permissions bullet), `docs/drafthorse/framework/handover.md:26` ("the grants transfer to whatever executes the handover"), `extensions/skills/drafthorse/assets/SKILL-template.md:8` (the permissions frontmatter comment), and `extensions/skills/drafthorse/SKILL.md:26` (The Scaffold reference, "which transfer to a sub-agent that invokes the skill" — found during the wave-3 pass). Evidence against treating it as settled: (a) the author reports never getting grant transfer to work reliably, and instead runs a harness-level "auto mode on" setting that grants automatically so permissions need no explicit call — a workaround, not a spec fix; (b) `git-box/SKILL.md` holds BOTH `Agent(git-robot)` and the three `Skill(dev-tools:agent-*)` grants while `git-robot.md` separately carries the same three in its own `tools:`, so one of the two paths is redundant and the corpus does not demonstrate which is load-bearing. Wave 3 deliberately left all three sites UNCHANGED to avoid a piecemeal mismatch. Decide the mechanism here and migrate the three together. NOTE: "auto mode on" is a harness setting, so it does not belong in framework prose — if the transfer claim cannot be substantiated, soften the sites to what is observable rather than documenting the workaround.
