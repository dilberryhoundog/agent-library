...
- `extensions/skills/drafthorse/assets/HANDOVER-template.md` — the draft comment block (lines 29–36) becomes the live quote block; delete the stale quote block (37–42) and the draft comment wrapper; collapse the optional problem step to a one-line pointer.
- `docs/drafthorse/framework/handover.md` — rewrite the "same universal preamble" claim (~line 57) to describe the handover variant and what differs; rewrite the "Globals join the master's global set" bullet in `## The sub-step model` to the parent-step scope; add the pointer to HANDOVER-template.md as the canonical creation path. Runs in the handover.md session with [plan-handover-location.md](plan-handover-location.md), [plan-frontmatter-stamp.md](plan-frontmatter-stamp.md), [plan-parent-child-vocab.md](plan-parent-child-vocab.md), and the [plan-utilities-term.md](plan-utilities-term.md) de-segmenting line.
- `extensions/skills/drafthorse/references/step-splitting.md` — carries the handover rules (lines ~35–43); align with the globals scope change.
