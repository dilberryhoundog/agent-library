...
| D-18 | Bare fragment; disk-primary links; Artifact publishing stays open                 | Shell → Form; Provenance → Origin gating                          |
| D-19 | Ship as a skill; landing directory settled here                                   | Skill; File layout                                                |
...
| path                                                                     | purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | size                                                                              |
|--------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
| `extensions/skills/artifact-suite/SKILL.md`                              | DraftHorse procedure: frontmatter, invariants, marker table, landing rule, token economy, ten steps, terms                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 260-320 lines                                                                     |
...
| `extensions/skills/artifact-suite/assets/verify.py`                      | Implements the static checks S1-S9 as subcommands of one script, run as `python3 $S/verify.py <page> --static`; also writes the doctype-wrapped copy V6a serves. Exits non-zero on the first failing check, naming it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | ~120 lines                                                                        |
...
| `plugins/chat-tools/skills/artifact-suite`                               | Symlink `-> ../../../extensions/skills/artifact-suite`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | symlink                                                                           |
...
`assets/shell.html` is a bare fragment: no `<!doctype>`, `<html>`, `<head>` or `<body>` (D-18). Chrome builds the missing structure from `file://`; the Artifact tool supplies its own skeleton, so one file serves both (AR-11). The two targets differ in rendering mode: from `file://` the fragment is quirks mode (`document.compatMode === "BackCompat"`), under the Artifact wrapper it is standards mode. Quirks mode differs from standards mode in ways the shell must not rely on: class and id selectors match case-insensitively, table cells do not inherit font-size or line-height, percentage heights resolve against an auto-height parent, and images in table cells sit on a different baseline. The shell therefore uses lowercase class and id names only, sets font and line-height on `td`/`th` explicitly, uses no percentage heights, and puts no image in a table. A cheaper cure is open in Unresolved questions: ship `<!doctype html>` as line 1 and strip it with one `sed` at publish time. Non-ASCII
glyphs in CSS `content` are written as escapes (`\2212` for the diff minus, `\00B7` for the eyebrow separator) because the fragment's `<meta charset>` is inert under the wrapper. Line 1 is the version stamp and the editing rule: `<!-- artifact-suite shell v1. Write only between the @markers and the class on .shell; everything else is shared chrome. -->`. Verification reads the stamp.

A page is created by `cp extensions/skills/artifact-suite/assets/shell.html dev/workspace/artifacts/<id>.html`, never by writing chrome (AR-1, AR-15). The agent never opens the shell or a page copy with Read or Edit; the page's own state is read back only through `splice.py … show`. Splices are made with `assets/splice.py` under Bash, never with Edit or Write: both tools refuse a path they have not Read in the same session, and reading the copy would re-read the whole shell.
...
|--------------------|---------------------------------------------------|---------------------|-------------------------------------------------------------------------------------------------------------|
| `@doc-title`       | the `<title>` element                             | replace placeholder | the page title, equal to `#page-meta.title` (a real tag: the Artifact tool scans only the first 8KB for it) |
...
Three buttons in `.themeset` carrying `data-theme-set="system|light|dark"` with `aria-pressed` (D-4, DS-7). System removes `data-theme` and the stored key so the three-tier cascade governs again. The pre-paint script is the first `<script>` in the fragment, above `<style>` (DS-5, AR-11, PR-3):
...
`fingerprintOf(card)`: clone the card, remove `.respond`, `.changed` and `.chip.earlier`, then `hashOf(normalise(clone.textContent) + "|" + (card.dataset.tag || "") + "|" + (card.dataset.verdicts || ""))`. `normalise` collapses whitespace runs and trims; `hashOf` is prompt-builder's DJB2 (lines 1693-1697). Per card, never per file (AR-7). Normalised text rather than innerHTML so re-indenting during an unrelated edit does not read as a change; every prose, option and tag change is still caught.
...
`capabilityGate()` removes every `[data-needs="fs"]` element when `CAN_FS` is false (ST-12) and renders `#storage-note` when `CAN_LS` is false. No File System Access affordance ships in this release (AR-17). No `prompt()` or `alert()` anywhere. The required surface is localStorage plus clipboard plus inline CSS/JS, which works from `file://` and under the Artifact CSP alike.
...
| `hashOf(s)` / `normalise(s)` / `fingerprintOf(card)`                                                         | DJB2; whitespace collapse; per-card fingerprint                                                                                                                                                                                                                                                                                                                                                                                       |
...
```css
:root[data-origin="web"] .ref-disk, .ref--nodisk .ref-disk, .ref--nodisk .ref-editor { display: none; }
...
| origin `web`                                                             | Disk chip hidden, GitHub primary                                      |
...
DraftHorse format (`harness-format: DraftHorse`), matching `classroom`, `corpus-sweep`, `versioning`. Frontmatter:
...
Both invocation fields omitted: model-invocable and user-invocable, so the agent reaches for it before hand-authoring chrome (AR-1). No `rm` grant; no Edit or Write grant, since every write to a page goes through `splice.py` (Shell → Form). The static checks run through `assets/verify.py` under the `python3` grant; `grep`, `diff` and `wc` are granted for the acceptance test's byte count, the path check's line count and ad hoc confirmation, and `browser_take_screenshot` for V6a. Body: global invariants (write only between markers and the class on `.shell`; never Read the shell or the page copy; ids permanent; `<\/` rule), inline references (marker table, landing rule, token economy, six-kind mapping), ten steps, terms.

### Plugin placement and release

Symlink `plugins/chat-tools/skills/artifact-suite -> ../../../extensions/skills/artifact-suite`. `chat-tools` ("shape context, create documents") already holds `markdown`, whose output this is the HTML counterpart of; `agent-tools` is harness-extension authoring and `dev-tools` is git and releases. Release bumps `chat-tools` 2.1.2 → 2.2.0 through `/dev-tools:versioning`, which resolves symlink targets at runtime (`.claude/rules/versioning.md`). Plugin assets run from the last installed release, so through Stage 3 the skill is executed by hand from the working tree; the release is cut only after the acceptance test passes.

### Build procedure (the ten steps)

Each step is its own DraftHorse heading with its own start and finish conditions; the error step stands alone as step 10.
...
- **S1** no `<!doctype`, `<html`, `<head` or `<body` tag in the file (D-18; `<header` is not a match); line 1 carries the shell stamp.
...
- **V6a** the same fixture is served twice from the localhost server — bare, and as a copy wrapped in `<!doctype html><html><head></head><body>…</body></html>` that `verify.py` writes beside it — and `document.compatMode` reads `BackCompat` for the bare copy and `CSS1Compat` for the wrapped copy, with identical `browser_take_screenshot` captures at 1100px and identical `getBoundingClientRect()` widths of `.shell`, `main` and `.card`. The wrapper stands in for the Artifact tool's skeleton; no page is published to run this check (D-18).
...
- **V12** mutate one card's text through the page, reload: `.changed` on that card only, all other answers intact (D-7, AR-7).
...
| WP-10 | symlink and `chat-tools` 2.2.0 via `/dev-tools:versioning`                                                                                                                                                                                               | WP-9                            | `/chat-tools:artifact-suite` is listed after install                                                                                                                                                                                                               |

WP-4 runs once WP-3 lands; WP-5 depends only on WP-1 and WP-2, so it runs alongside WP-3. Nothing is released before WP-9.
...
- `:has()` with no fallback beyond the verdict floor (DS-18): acceptable for Chrome; re-check if published through the Artifact tool.
- Quirks mode from `file://` (D-18): the shell avoids the four quirks differences named under Shell → Form; a doctype line stripped at publish would remove the risk outright.
- Absolute checkout path in `#page-meta.provenance.root` (D-11): a page published through the Artifact tool exposes the home directory name.
...
- Shell budget set at 36KB hard / 32KB target over AR-1's 14-18KB — accept?
- `Bash(kill:*)` in the skill's allowed-tools, or the user starts and stops the verification server?
- Do `tests/fixtures/` ship inside the released plugin payload?
- D-18 fragment renders in quirks mode from disk; keep the fragment, or ship `<!doctype html>` on line 1 and strip it at publish?
- Edit and Write are absent from the skill's allowed-tools; every page write goes through `assets/splice.py` and the agent never Reads the shell or a page copy — confirm this tool set (kickoff names none).
