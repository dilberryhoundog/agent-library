---
name: workspace overview
description: Core workspace and project structure
---

dev/workspace
├── context
│   ├── .keep
│   └── tree.md
├── filebox
│   ├── drafthorse-plan.md
│   ├── drafthorse-v2-exploration.md
│   ├── git-box-plan.md
│   ├── README.txt
│   ├── surfaces-draft.md
│   ├── verb-skills-plan.md
│   └── versioning-plan.md
├── history
│   └── .keep
├── plans
│   ├── .keep
│   ├── 00-INDEX.md
│   ├── architectural.md
│   ├── new-concepts-discussion.md
│   ├── plan-audit-fixes.md
│   ├── plan-child-skills.md
│   ├── plan-classroom-saddler-fixes.md
│   ├── plan-condition-links.md
│   ├── plan-conditions-carry-routing.md
│   ├── plan-config-wiring.md
│   ├── plan-decision-slot.md
│   ├── plan-drafthorse-rule.md
│   ├── plan-frontmatter-stamp.md
│   ├── plan-gates-are-compound.md
│   ├── plan-half-applied-dispositions.md
│   ├── plan-handover-citation.md
│   ├── plan-handover-location.md
│   ├── plan-handover-template.md
│   ├── plan-machinery-headings.md
│   ├── plan-mechanical-fixes.md
│   ├── plan-naming-not-explaining.md
│   ├── plan-parent-child-vocab.md
│   ├── plan-preamble-rewrite.md
│   ├── plan-reference-notation.md
│   ├── plan-sharp-prose.md
│   ├── plan-spec-check-saddler.md
│   ├── plan-steps-are-universal.md
│   ├── plan-surfaces-doc.md
│   ├── plan-terms-notation.md
│   ├── plan-utilities-term.md
│   ├── prd.md
│   ├── report-call-sites.md
│   ├── report-grammar-typos.md
│   ├── report-new-concepts.md
│   ├── report-prose-review.md
│   ├── report-todos.md
│   └── todos-discussion.md
├── prompts
│   ├── .keep
│   ├── discover_prompt.md
│   ├── README.md
│   └── research_prompt.md
├── research
│   └── .keep
├── reviews
│   ├── .keep
│   └── independent-audit.md
├── tasks
│   └── .keep
├── README.md
├── workspace-config.yml
└── WORKSPACE.md
.claude-plugin
└── marketplace.json
docs
├── claude_docs
│   ├── create_plugins.md
│   ├── marketplaces.md
│   ├── plugin_reference.md
│   ├── plugin_versions.md
│   └── skills_extend.md
├── drafthorse
│   ├── framework
│   │   ├── conventions.md
│   │   ├── handover.md
│   │   ├── notation.md
│   │   ├── README.md
│   │   ├── references.md
│   │   ├── scaffold.md
│   │   ├── steps.md
│   │   └── surfaces.md
│   └── drafthorse-spec-check.md
├── agnostic-documents.md
├── classroom-skeleton.md
├── create_symlink.md
└── user-configuration.md
extensions
├── agents
│   ├── breaking-change-detector.md
│   ├── course-researcher.md
│   ├── doc-reviewer.md
│   ├── drafthorse-saddler.md
│   ├── git-robot.md
│   └── readme.md
├── rules
│   ├── code-comments.md
│   ├── DraftHorse.md
│   ├── durable-documents.md
│   └── STYLE.md
└── skills
    ├── classroom
    │   ├── references
    │   │   ├── pedagogy
    │   │   │   └── dyslexia-kinetic.md
    │   │   ├── students
    │   │   │   ├── _template.md
    │   │   │   └── example-learner.md
    │   │   ├── curriculum-spines.md
    │   │   └── vetted-video-channels.md
    │   ├── templates
    │   │   ├── blocks
    │   │   │   ├── confidence-stars.html
    │   │   │   ├── craft-activity.html
    │   │   │   ├── real-world-challenge.html
    │   │   │   ├── reflection-page.html
    │   │   │   └── video-library-page.html
    │   │   ├── course-structures
    │   │   │   ├── spine-plus-companion.md
    │   │   │   ├── spiral-units.md
    │   │   │   └── weeks-per-theme-plus-capstone.md
    │   │   ├── documents
    │   │   │   ├── answer-key-a4.html
    │   │   │   ├── certificate.html
    │   │   │   ├── competency-report.html
    │   │   │   ├── parent-lesson-guide.html
    │   │   │   ├── review-document.html
    │   │   │   ├── scope-and-sequence.html
    │   │   │   └── workbook-a4.html
    │   │   └── lesson-structures
    │   │       ├── early-learner-multisensory.md
    │   │       ├── explore-investigate.md
    │   │       ├── studio-practice.md
    │   │       ├── timed-session.md
    │   │       └── watch-concept-notes-practice-reflect.md
    │   ├── deliver-without-renderer-handover.md
    │   ├── mark-review-handover.md
    │   ├── media-processing-handover.md
    │   ├── setup-handover.md
    │   └── SKILL.md
    ├── drafthorse
    │   ├── assets
    │   │   ├── HANDOVER-template.md
    │   │   └── SKILL-template.md
    │   ├── references
    │   │   ├── collecting-references.md
    │   │   ├── condition-writing.md
    │   │   ├── README.md
    │   │   └── step-splitting.md
    │   └── SKILL.md
    ├── git
    │   ├── agent-commit
    │   │   └── SKILL.md
    │   ├── agent-push
    │   │   └── SKILL.md
    │   └── agent-switch
    │       └── SKILL.md
    ├── git-box
    │   ├── workflows
    │   │   ├── commit-and-push.md
    │   │   ├── multi-commit-and-push.md
    │   │   └── README.md
    │   └── SKILL.md
    ├── magic-reply
    │   ├── references
    │   │   ├── check-working.txt
    │   │   ├── claude-space.txt
    │   │   ├── show-chat-summary.txt
    │   │   ├── show-context.txt
    │   │   ├── show-difficulties.txt
    │   │   ├── show-options.txt
    │   │   ├── show-strategy.txt
    │   │   └── show-working.txt
    │   └── SKILL.md
    ├── markdown
    │   ├── references
    │   │   └── example.md
    │   └── SKILL.md
    └── versioning
        ├── assets
        │   └── CHANGELOG-template.md
        ├── references
        │   └── config-template.md
        └── SKILL.md
plugins
├── agent-tools
│   ├── .claude-plugin
│   │   └── plugin.json
│   ├── agents
│   │   └── drafthorse-saddler.md -> ../../../extensions/agents/drafthorse-saddler.md
│   ├── skills
│   │   └── drafthorse -> ../../../extensions/skills/drafthorse
│   └── CHANGELOG.md
├── chat-tools
│   ├── .claude-plugin
│   │   └── plugin.json
│   ├── agents
│   │   └── doc-reviewer.md -> ../../../extensions/agents/doc-reviewer.md
│   ├── skills
│   │   ├── magic-reply -> ../../../extensions/skills/magic-reply
│   │   └── markdown -> ../../../extensions/skills/markdown
│   └── CHANGELOG.md
├── classroom
│   ├── .claude-plugin
│   │   └── plugin.json
│   ├── agents
│   │   └── course-researcher.md -> ../../../extensions/agents/course-researcher.md
│   ├── mcp
│   │   ├── test
│   │   │   ├── fixtures
│   │   │   │   ├── annotation-band.html
│   │   │   │   ├── band-with-cover.html
│   │   │   │   ├── bleed-cover.html
│   │   │   │   ├── bleed-then-content.html
│   │   │   │   ├── bleed-wide.html
│   │   │   │   ├── block-unbreakable.html
│   │   │   │   ├── content-long.html
│   │   │   │   ├── content-short.html
│   │   │   │   ├── overflow-wide.html
│   │   │   │   ├── override-landscape.html
│   │   │   │   ├── pages-ten.html
│   │   │   │   ├── pages-three.html
│   │   │   │   ├── svg-overflow.html
│   │   │   │   └── table-long.html
│   │   │   ├── README.md
│   │   │   └── run-tests.js
│   │   ├── css-inspect.js
│   │   ├── package.json
│   │   ├── pdf-inspect.js
│   │   ├── print-base.css
│   │   └── server.js
│   ├── scripts
│   │   └── install-deps.sh
│   ├── skills
│   │   └── classroom -> ../../../extensions/skills/classroom
│   ├── templates
│   │   ├── .claude
│   │   │   └── rules
│   │   │       ├── classroom-signal.md
│   │   │       └── classroom.md
│   │   ├── courses
│   │   ├── students
│   │   │   └── README.md
│   │   ├── CLAUDE.md
│   │   └── global-requirements.md
│   ├── .mcp.json
│   └── CHANGELOG.md
└── dev-tools
    ├── .claude-plugin
    │   └── plugin.json
    ├── agents
    │   ├── breaking-change-detector.md -> ../../../extensions/agents/breaking-change-detector.md
    │   └── git-robot.md -> ../../../extensions/agents/git-robot.md
    ├── skills
    │   ├── agent-commit -> ../../../extensions/skills/git/agent-commit
    │   ├── agent-push -> ../../../extensions/skills/git/agent-push
    │   ├── agent-switch -> ../../../extensions/skills/git/agent-switch
    │   ├── git-box -> ../../../extensions/skills/git-box
    │   └── versioning -> ../../../extensions/skills/versioning
    └── CHANGELOG.md

78 directories, 186 files
