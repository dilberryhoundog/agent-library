...
description: Research course material for a classroom build. Given a subject, the learner's constraints, and the family's sourcing constants, it draws a complete unit picture from the agent's own knowledge and live web search, ties each unit to verified free and purchased material, and returns a structured candidate set. Use when a classroom build needs grounding research or source material the user has not supplied in hand. Read-only and non-interactive — it never writes files or talks to the user; the parent presents the candidates, the user chooses, and the parent records the choice.
