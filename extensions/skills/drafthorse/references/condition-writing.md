...
How to write the conditions and the *suggested next actions* guidance that frame every step. Together they carry all the routing a wired graph would — a weak condition is a broken edge. Responsibility is strictly divided: start conditions carry the routing, finished conditions carry only their own step's completion criteria, and *suggested next actions* is the one place a step may name another step.
...
- **Own step only** — never mention another step, narrate where the flow goes, or issue instructions. Routing lives in the other steps' start conditions and in *suggested next actions*; work lives in the engagement. A routing fact stated in two homes drifts.
...
- **Never contradict a start condition** — when the finished condition has failure outcomes, the suggestion either covers them ("a recorded failure moves to reporting the result; otherwise…") or stays silent about them; a happy-path-only pointer that sends an ended run onward overrides the destination's own refusal.
