# Versioning Config Template

The shape of a repository's versioning config, and the facts needed to fill it in. The config externalises everything repo-specific so the versioning skill stays universal.
...
- **Single project** (one deliverable): one unit. Manifest candidates — the file holding the **literal version string**: `package.json`, `pyproject.toml`, `Cargo.toml`, `.claude-plugin/plugin.json`, a `VERSION` file. Beware delegation: some ecosystems reference the version from elsewhere (a gemspec reading `lib/<gem>/version.rb`, a build file reading a constant) — the manifest is the file and field that contain the literal, not the reference. Tag pattern is plain `v{version}`.
...
- Changelog default location: `CHANGELOG.md` inside the unit's directory; repository root for a repo-level unit.
