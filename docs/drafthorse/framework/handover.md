...
- **No required exit steps.** A skill needs a success exit (whose completion ends the run) and an error step. A handover needs neither: when no handover step is left in play, control simply returns to the parent step, and a failure falls to the parent document's error step. A handover *may* carry a local error step to surface something to the user mid-work, but it owes the parent no handback and needs no terminal success step whose only job is to report an outcome.
