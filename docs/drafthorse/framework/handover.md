...
Step anatomy, conditions, and in-play semantics are unchanged (see [Steps](steps.md)). A handover opens its steps section with the **handover-variant preamble**, not the universal one: the universal preamble sends a step that cannot be completed to the error drain, and inside a handover there is no error drain — a failure falls to the parent document's problem step.
The variant states the same reading model in the terms that are true here: child steps of a parent step, no exits of their own, error handling owned by the parent. Both preambles are legitimate; a document is read against the one that fits it. [HANDOVER-template.md](../../../extensions/skills/drafthorse/assets/HANDOVER-template.md) carries the canonical variant text and is the creation path for a new handover.
